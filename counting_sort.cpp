...
void countingSort(vector<int>& listaNumeros) {
    int n = listaNumeros.size();

    // Encontrar el elemento máximo en la lista
    int maxElemento = *max_element(listaNumeros.begin(), listaNumeros.end());

    // Crear un arreglo de conteo para almacenar la cantidad de cada elemento
    vector<int> conteo(maxElemento + 1, 0);

    // Almacenar la cantidad de cada elemento en el arreglo de conteo
    for (int i = 0; i < n; i++) {
        conteo[listaNumeros[i]]++;
    }

    // Modificar el arreglo de conteo para almacenar la posición real de cada elemento
    for (int i = 1; i <= maxElemento; i++) {
        conteo[i] += conteo[i - 1];
    }

    // Crear un arreglo temporal para almacenar el resultado ordenado
    vector<int> resultado(n);

    // Construir el arreglo de resultado
    for (int i = n - 1; i >= 0; i--) {
        resultado[conteo[listaNumeros[i]] - 1] = listaNumeros[i];
        conteo[listaNumeros[i]]--;
    }

    // Copiar los elementos ordenados de vuelta a la lista original
    for (int i = 0; i < n; i++) {
        listaNumeros[i] = resultado[i];
    }
}
...
int main(){
    ...
    std::ofstream archivo("lista_tiempos_countingsort.txt");
